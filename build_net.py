...
def build_Conv1D():
    model = tf.keras.models.Sequential()
    model.add(tf.keras.layers.Conv1D(16, 20, activation='relu', input_shape=(500, 1, )))
...
def build_combo():
    ...
    y = tf.keras.layers.Conv1D(16, 20, activation='relu')(inputConv)
    y = tf.keras.layers.MaxPooling1D(3)(y)
    y = tf.keras.layers.Conv1D(32, 10, activation='relu')(y)
    y = tf.keras.layers.MaxPooling1D(3)(y)
    ...
    out = tf.keras.layers.Dense(300, activation='relu')(concat)
